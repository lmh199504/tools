...
export function nextFocus(e, autoComInp) {
...
export function resetTableHeight(target, elConditions, exHeight) {
  const wrapperH = document.body.clientHeight
  const conditionsH = elConditions.clientHeight
  const paginationH = 120 + 140 + 60 + exHeight
  const tH = wrapperH - conditionsH - paginationH
  return tH
}

// 重置对象，把键值对的值都变成null,通常此方法用于重置查询条件
...
/**
 * 处理值集，增加label addby 李明悦 2022.03.08
*/
export function filterLovValues(lovValues) {
  if (!Object.keys(lovValues).length) return {}
  for (const key in lovValues) {
    (lovValues[key] || []).forEach(item => {
